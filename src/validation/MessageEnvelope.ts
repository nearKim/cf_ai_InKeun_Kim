...
const MessageEnvelopeSchema = Schema.Struct({
...
export const parseEnvelope = (
  raw: string
): Effect.Effect<MessageEnvelope, MessageEnvelopeParseError> =>
  pipe(
    Effect.try({
...
export const createEnvelope = (params: {
  id?: string
  payload: string
  timestamp?: number
}): MessageEnvelope =>
...
export const serializeEnvelope = (envelope: MessageEnvelope): string =>
